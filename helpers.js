...
export function createKuiperBelt(count = 8000) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const theta = Math.random() * Math.PI * 2;
    const yOffset = (Math.random() - 0.5) * (Math.random() * 4); 
    ...
    const colorChoice = Math.random();
    if (colorChoice < 0.6) {
      colors[i3] = 0.8; colors[i3 + 1] = 0.8; colors[i3 + 2] = 0.9; 
    } else {
      colors[i3] = 0.6; colors[i3 + 1] = 0.5; colors[i3 + 2] = 0.5; 
    }
...
export function createOortCloud(count = 5000) {
  const geometry = new THREE.BufferGeometry();
  const positions = new Float32Array(count * 3);
  const colors = new Float32Array(count * 3);
  const sizes = new Float32Array(count);
  ...
  for (let i = 0; i < count; i++) {
    ...
    colors[i3] = 0.7; colors[i3 + 1] = 0.7; colors[i3 + 2] = 0.8;
